...
class MCoreAVLMModel(MCoreLLaVAModel):
    ...
    def __init__(
        ...
        # TODO - Add support for audio model
        self.freeze(
    ...
    def forward(
        ...
        print(
            f"final_labels (non_negative_indices)[0].tolist(): {(final_labels[0]>=0).nonzero(as_tuple=True)[0].tolist()}"
        )
        print(f"final_loss_mask.shape: {final_loss_mask.shape}")
        print(
            f"final_loss_mask (non_zero_indices)[0].tolist(): {(final_loss_mask[0]>0).nonzero(as_tuple=True)[0].tolist()}"
        )
